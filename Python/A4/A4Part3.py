...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    binFreq = np.arange(N) * (fs/N)
    k = 1
    hiBin = loBin = 0
    while binFreq[k] < 3000:
        k += 1
    
        loBin = k
    #print 'loBin =', loBin

    while binFreq[k] < 10000:
        k += 1
        
        hiBin = k
    #print 'hiBin =', hiBin

    for i in range (np.shape(mX)[0]):  
        for k in range (1, loBin):
            db2lin = np.sqrt(math.pow(10,mX[i,k]/10))
            absX1 = np.append(absX1, math.pow(abs(db2lin),2))
            
        for k in range (loBin, hiBin):
            db2lin = np.sqrt(math.pow(10,mX[i,k]/10))
            absX2 = np.append(absX2, math.pow(abs(db2lin),2))
            
        E1 = np.append(E1, sum(absX1))
        E2 = np.append(E2, sum(absX2))

    engEnv = np.column_stack((E1,E2))
