...
def minFreqEstErr(inputFile, f):
    ...
    while (abs(fEst - f) > error):
        M = 100*k+1 #compute the first window size calculation
        
        #check if frame >= sine period (for detection)
        if (M <= (fs/f)):
            k = k + 1
            M = 100*k+1
        
        #check if N > M (due for the DFT function)    
        while (M >= N):
            j = j + 1
            N = pow(2,j)
