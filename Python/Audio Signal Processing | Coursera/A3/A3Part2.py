...
def optimalZeropad(x, fs, f):
    ...
    ## Your code here
    M=np.size(x)
    p=fs/f #number of samples per period
    pnum=M/float(p) #number of periods in M
    k = np.ceil(pnum) #ceil to the next integer
    S = k*p #number of samples per k-periods
    N = M + (S-M) #N size in samples (zero-padding included)
