...
def testRealEven(x):
    ...
    X1 = fft(fftbuffer)
    X2 = fft(np.fft.fftshift(fftbuffer))
    
    #simmetry condition over the magnitude spectrum
    for i in range (0,M/2+1):
        if (abs(X1.real[i]-X2.real[i]) < exp(1e-6)):
            isReal = True
        else:
            isReal = False
    
    #symmetry condition over the imaginary part
    for i in range (0,M/2+1):
        if (abs(X1.imag[i]) < exp(1e-12)):
            isEven = True
        else:
            isEven = False
    
    #check whether the input signal is real and even
    isRealEven = bool(np.logical_and(isReal, isEven))
    y = (isRealEven,fftbuffer,X1)
