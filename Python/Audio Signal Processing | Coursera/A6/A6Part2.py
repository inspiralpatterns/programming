...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    ### your code here
    sigma = np.zeros(len(f0)) #standard deviation array
    idx = np.array([]) 
    ranges = np.array([])
    segments = np.array([])

    # 1. convert f0 values from Hz to Cents (as described in pdf document)
    f0cent = 1200*np.log2(f0+eps/55);

    #2. create an array containing standard deviation of last winStable samples
    for i in range (np.size(f0cent)):
        #if (i - winStable > 0 and i + 1 < np.size(f0cent) - 1):        
            sigma[i] = np.std(f0cent[i-(winStable-1):i+1])
        
    #3. apply threshold on standard deviation values to find indexes of the stable points in melody   
    sgmIdx = np.where(sigma < stdThsld)
    sgmIdx = sgmIdx[0] #because sgmIdx is a tuplet
    
    #4. create segments of continuous stable points such that consecutive stable points belong to same segment
    for i in range (np.size(sgmIdx)):
        if (abs(sgmIdx[i]-sgmIdx[i-1]) != 1): #find correlation
            idx = np.append(idx,sgmIdx[i])    #index of each discontinuity
    
    for i in range (1,len(idx)):
        ranges = np.append(ranges, (idx[i-1],idx[i]-1))
                
    k = np.size(ranges)
    ranges = np.reshape(ranges,(k/2,2))
    
    #5. apply segment filtering, i.e. remove segments with are < minNoteDur in length    
    for i in range (len(ranges)):
        if (abs(ranges[i,0]-ranges[i,1])*H > minNoteDur*fs):
            segments = np.append(segments, (ranges[i,0],ranges[i,1]))
    
    k = np.size(segments)
    segments = np.reshape(segments,(k/2,2)) #reshaping segments array 
